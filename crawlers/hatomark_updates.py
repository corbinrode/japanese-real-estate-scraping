...
from helpers import translate_text, save_image, setup_logger
...
# Set up logger
logger = setup_logger('hatomark_updates', 'hatomark_updates')

MAX_RETRIES = 5
INITIAL_BACKOFF = 30  # in seconds
MAX_WORKERS = 8  # Parallel workers for processing documents
...
def scrape_additional_data(document):
    ...
    if len(existing_images) == 1:
        ...
        images = []
        for div in soup.find_all('div', class_='slick-img'):
        ...
        # Remove duplicates and sort by data-index
        unique_images = list(dict.fromkeys(images))
        sorted_images = sorted(unique_images, key=lambda x: x[0])
        
        # Extract only the image URLs, skip the first one
        if len(sorted_images) > 1:
            additional_image_urls = [url for _, url in sorted_images[1:]]
            
            new_image_paths = []
            for img_url in additional_image_urls:
                file_name = "{}.jpg".format(uuid4())
                folder = os.path.join("images", "hatomark", property_id_str)
            ...
            if new_image_paths:
                ...
                logger.info(f"No additional images could be saved for: {link}")
        else:
...
def main():
    ...
    logger.info("Starting parallel update process for hatomark collection...")
