...
const generateMoreListings = (): RealEstateListing[] => {
  ...
  for (let i = 11; i <= 150; i++) {
    ...
    additionalListings.push({
      id: i.toString(),
      title: `Property ${i} in ${prefecture.display}`,
      prefecture: prefecture.value,
      ...(hasPrice && { price }),
      ...(hasFloorPlan && { floorPlan }),
      ...(hasArea && { area: area.toString() }),
      landArea: landArea.toString(),
      ...(hasAddress && { address: `District ${i}, ${prefecture.display}` }),
      imageUrl: `https://images.unsplash.com/${randomImageId}?w=400&h=300&fit=crop&q=80&id=${i}`,
      yearBuilt: hasYearBuilt ? yearBuilt.toString() : undefined,
      listingDate: date.toISOString(),
      description: `Beautiful property located in ${prefecture.display}. Perfect for modern living with great amenities.`,
      features: ["Modern amenities", "Great location", "Well maintained", "Good value"],
      propertyType: hasPropertyType ? propertyType : undefined,
      transportation: hasTransportation ? selectedTransports.join(" / ") : undefined,
      buildingStructure: hasBuildingStructure ? `${material}, ${floors} floors` : undefined
    });
  }
